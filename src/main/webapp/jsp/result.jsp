        	...
        	<div class="col-lg-4">
            	<form role="form" action="query">
                	<div class="input-group">
                    	<input type="text" class="form-control" id="keywords" name="keywords" value="${requestScope.keywords }">
    ...
    <c:forEach var="page" items="${requestScope.doclist }">
    	<div class="row">
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 col-lg-offset-1 col-md-offset-1 col-sm-offset-1 col-xs-offset-1">
                <div><a href="${page.url }" class="title" target="_blanck">${page.title }</a></div>
                <div class="content">${page.content }"</div>
                <div><a href="${page.url }" class="link" target="_blanck">${page.url }</a></div>
            </div>
        </div>
        <br>
    </c:forEach>
        ...
        <div class="col-lg-11 col-md-11 col-sm-11 col-xs-11 col-lg-offset-1 col-md-offset-1 col-sm-offset-1 col-xs-offset-1">
            <ul class="pagination pagination-lg">
            	<c:if test="${requestScope.pageInfo.currentPage > 1}">
            		<li><a href="${pageContext.request.contextPath}/query?keywords=${requestScope.keywords}&pageId=${requestScope.pageInfo.currentPage-1}">上一页</a></li>
            	</c:if>
   					...
   					<c:if test="${(requestScope.pageInfo.startPage + i) <= requestScope.pageInfo.totalPage}">
   						<c:choose>
   							<c:when test="${requestScope.pageInfo.currentPage == (requestScope.pageInfo.startPage + i) }">
   								<li  class="active"><a href="${pageContext.request.contextPath}/query?keywords=${requestScope.keywords}&pageId=${requestScope.pageInfo.startPage + i}">${requestScope.pageInfo.startPage + i}</a></li>
   							</c:when>
   							<c:otherwise>
   								<li><a href="${pageContext.request.contextPath}/query?keywords=${requestScope.keywords}&pageId=${requestScope.pageInfo.startPage + i}">${requestScope.pageInfo.startPage + i}</a></li>
   							</c:otherwise>
   						</c:choose>
   					</c:if>
				</c:forEach>
       			<c:if test="${requestScope.pageInfo.currentPage < requestScope.pageInfo.totalPage }">
       				<li><a href="${pageContext.request.contextPath}/query?keywords=${requestScope.keywords}&pageId=${requestScope.pageInfo.currentPage+1}">下一页</a></li>
            	</c:if>
            </ul>
        </div>
    </div>
